...
FUZZ_TARGETS=(gca_small gca_octopus range_medium range_octopus range_small)
...
for t in "${FUZZ_TARGETS[@]}"; do
  ...
  cp "$bin" "/mayhem/$t"
  echo "built /mayhem/$t"
  # Reclaim disk immediately: the ASan+debuginfo=2 release binary is large and is already
  # copied out to /mayhem/$t. "$bin" is hardlinked to its deps/$t-<hash> executable (not a
  # .rlib/.rmeta — those are the shared dependency graph and stay cached for later targets
  # in this loop), so both link names must go or the blocks never get freed.
  rm -f "$bin" "$bin.d"
  find "$FUZZ_DIR_ABS/target/$TRIPLE/release/deps" -maxdepth 1 -name "$t-*" ! -name "*.rlib" ! -name "*.rmeta" ! -name "*.d" -delete
done
